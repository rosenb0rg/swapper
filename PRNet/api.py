...
class PRN:
    ...
    def __init__(self, is_dlib = False, prefix = '.'):
        ...
        #---- load detectors
        # change this to whatever directory you put the Data folder in
        data_dir = os.path.join('.', 'Data')


        if is_dlib:
            import dlib
            detector_path = os.path.join(data_dir, 'net-data','mmod_human_face_detector.dat')
            self.face_detector = dlib.cnn_face_detection_model_v1(
                    detector_path)

        #---- load PRN 
        self.pos_predictor = PosPrediction(self.resolution_inp, self.resolution_op)
        prn_path = os.path.join(data_dir, 'net-data','256_256_resfcn256_weight')
        ...
        # uv file
        uv_dir = os.path.join(data_dir, 'uv-data')

        self.uv_kpt_ind = np.loadtxt(os.path.join(uv_dir, 'uv_kpt_ind.txt')).astype(np.int32) # 2 x 68 get kpt
        self.face_ind = np.loadtxt(os.path.join(uv_dir, 'face_ind.txt')).astype(np.int32) # get valid vertices in the pos map
        self.triangles = np.loadtxt(os.path.join(uv_dir, 'triangles.txt')).astype(np.int32) # ntri x 3
